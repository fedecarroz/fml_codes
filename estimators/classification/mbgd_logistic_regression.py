...
class MBGDLogisticRegression(BaseEstimator):
    def __init__(self, batch_size: int, n_features=1, n_steps=2000, alpha=1e-2, penalty="l2", lmd=0):
        self.b = batch_size
    ...
    def fit(self, x_train, y_train, x_val, y_val):
        ...
        for step in range(self.n_steps):
            for i in range(0, m, self.b):
                ...
                b_exp = np.dot(x_train[i:z], self.theta)
                b_pred = sigmoid(b_exp)
                ...
                self.theta -= self.alpha * (np.dot(x_train[i:z].T, b_error) + reg_gd) / self.b
